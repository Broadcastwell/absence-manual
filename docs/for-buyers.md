...
Apply the same standard to every provider. Ask to see the question, the complete answer and the source behind a finding. A name appearing as the publisher of a cited page is different from a vendor being recommended to the buyer. The research files linked below preserve the observations and their collection designs so readers can inspect the evidence themselves.
...
If you want this measured for your own category rather than done by hand, the [free 10-question check](https://audit.broadcastwell.com) runs ten buyer questions on one engine. The paid ladder is three rungs: the [Category Audit at $490 once](https://broadcastwell.com/buy/audit), the AI Visibility Diagnostic at $990 once (paused until our next Audit delivery), and the program at $13,500 per 90 days billed $4,500 monthly, booked by a call. Every figure comes from the [published method, v1.1](https://broadcastwell.com/methodology), and the ladder is set out at [broadcastwell.com/pricing](https://broadcastwell.com/pricing). The Diagnostic reports presence per question type: its 35 questions sit in four groups, shortlist, role, use case and evaluation, and the findings state where you are named in each, so being cited on use-case questions is never mistaken for being on the shortlist. Every finding names the sources visible in the observed answers and the changes worth testing first. AI answers vary; repeated observations report disagreement.
